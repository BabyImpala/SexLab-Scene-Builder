...
const INNER_H_GAP = 500; // NODE_WIDTH(240) + ~260px gutter (room for stubs + bends)
const INNER_V_GAP = 220; // NODE_HEIGHT(112) + ~108px gutter
const TRANS_V_GAP = 180;
 ...
 * }}
 */
export function layoutFamilyClusters(sceneGraph, rootId, nodeIds, { getName } = {}) {
  ...
  const clusterLayouts = familyNames.map((family) => {
    ...
    poseRest.forEach((id, i) => {
      ...
      const row = Math.floor(i / MAX_INNER_COLS);
    ...
    const poseRows = Math.max(1, Math.ceil(poseRest.length / MAX_INNER_COLS));
    const transY0 = Math.max(INNER_V_GAP, poseRows * INNER_V_GAP) + 24;
    transRest.forEach((id, i) => {
      const col = i % (MAX_INNER_COLS + 1);
      const row = Math.floor(i / (MAX_INNER_COLS + 1));
      innerPositions.set(id, {
        x: col * (INNER_H_GAP * 0.85),
        y: transY0 + row * TRANS_V_GAP,
      });
    });

    let maxX = NODE_WIDTH;
    let maxY = NODE_HEIGHT;
    for (const pos of innerPositions.values()) {
  ...
  for (const id of ids) {
    const dests = (sceneGraph[id]?.dest || []).filter((d) => idSet.has(d));
...
export function buildConnectionRows(
  ...
  for (const source of nodeIds) {
    for (const target of (sceneGraph[source]?.dest || []).filter((d) => idSet.has(d))) {
